...
class EvaluateEveryN(Callback):
    ...
    def __init__(self, every_n_epochs, num_epochs, batch_size, full_data=None):
        ...
        self.save_path = f'metrics/StudentModel_epochs{num_epochs}_batch{batch_size}'
    ...
    def on_epoch_end(self, epoch, logs=None):
        if (epoch + 1) % self.every_n_epochs == 0:
            ...
            predictions = self.model.predict(cregions_full)
...
def student_model():
    ...
    output = QDense(4,
                    ...
                    bias_quantizer=quantized_bits(8,1,1,alpha=1),
                    name="dense_out")(x)
    model = Model(inputs=input_tensor, outputs=output, name="model_student_misc_v2_ft20")
...
def main():
    ...
    save_path = f'metrics/StudentModel_epochs{num_epochs}_batch{batch_size}'
    ...
    labels = np.zeros_like(top_2_extrema)
    labels[:] = top_2_extrema
